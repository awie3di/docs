...
In the example above, you are redirecting the user to the `Login` action in an `Account` controller. The action redirects the user to Auth0's `/authorize` endpoint for authentication, as described in the [Authenticate the user](#authenticate-the-user) step.
